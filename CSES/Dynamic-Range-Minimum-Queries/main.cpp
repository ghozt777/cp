/*
    ...
    Time: Mon Aug 22 13:41:44 IST 2022
	Link to problem / contest : https://cses.fi/problemset/task/1649
*/
...
class SegTree{
	...
	ll query_helper(ll start,ll end,ll v){
		Node &curr=t[v];
		if(curr.start>end||curr.end<start) return LLONG_MAX;
		if(curr.start>=start&&curr.end<=end) return curr.val;
		return min(query_helper(start,end,2*v),query_helper(start,end,2*v+1));
	}
